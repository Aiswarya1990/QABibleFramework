...
public class CaptureScreenshotforFailedTestCase {
	...
	public void captureScreenShotForFailedTestcase(WebDriver driver, String name) throws IOException 
	{
		...
		TakesScreenshot scrShot = (TakesScreenshot) driver; //TakesScreenshot is an interface in Selenium
		
		//file is a Java Class to handle files n folders in our system //using getScreenshotAs function we will store
		...
		if (!f1.exists()) {
			f1.mkdirs();// mkdir(make directory  in java) --> will create folder using java 
		}
